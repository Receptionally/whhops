-- Drop existing seller_updates table and related objects
drop table if exists public.seller_updates cascade;
drop function if exists approve_seller_update cascade;

-- Create seller_updates table with proper schema
...
create or replace function update_seller_updates_timestamp()
returns trigger as $$
begin
...
create trigger update_seller_updates_timestamp
    before update on public.seller_updates
    for each row
    execute function update_seller_updates_timestamp();
...
create policy "Enable full access for everyone"
    on public.seller_updates
    using (true)
    with check (true);

-- Grant permissions
grant usage on schema public to anon, authenticated;
grant all privileges on public.seller_updates to anon, authenticated;
grant execute on function approve_seller_update to anon, authenticated;
