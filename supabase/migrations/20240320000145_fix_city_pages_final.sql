...
create policy "Enable insert access for authenticated users"
    on public.city_pages for insert
    to authenticated
    with check (true);

create policy "Enable update access for authenticated users"
    on public.city_pages for update
    to authenticated
    using (true);

create policy "Enable delete access for authenticated users"
    on public.city_pages for delete
    to authenticated
    using (true);

-- Grant permissions
grant usage on schema public to anon, authenticated;
grant all privileges on public.city_pages to authenticated;
grant select on public.city_pages to anon;
grant execute on function approve_city_page to authenticated;
grant execute on function update_city_page_content to authenticated;
