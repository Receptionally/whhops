...
alter table public.sellers
  drop constraint if exists max_stacking_distance_check;

-- Add max_stacking_distance column if it doesn't exist
alter table public.sellers
  add column if not exists max_stacking_distance integer;

-- Update existing records to have a default value if they provide stacking
update public.sellers
set max_stacking_distance = 50
where provides_stacking = true 
and (max_stacking_distance is null or max_stacking_distance <= 0);

-- Add constraint to ensure max_stacking_distance is set when stacking is provided
alter table public.sellers
    ...
    set
        ...
        max_stacking_distance = case
            when (seller_update.updates->>'provides_stacking')::boolean = true then
                coalesce((seller_update.updates->>'max_stacking_distance')::integer, max_stacking_distance)
            else null
        end,
...
create index if not exists idx_sellers_stacking 
  on public.sellers(provides_stacking, max_stacking_distance);
