...
left join (
    select 
        ...
        sum(case when created_at >= current_timestamp - interval '7 days' then 1 else 0 end) as views_last_7_days
    from pageviews
    group by seller_id
...
left join (
    select 
        ...
        sum(case when created_at >= current_timestamp - interval '7 days' then 1 else 0 end) as appearances_last_7_days
