...
export function PaymentFormContent({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const deliveryFee = (amount / 100) - baseAmount;
      const stackingFee = includeStacking ? seller.stacking_fee_per_unit * quantity : 0;
      const totalAmount = baseAmount + deliveryFee + stackingFee;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <CustomerFormFields
        ...
        onPhoneChange={(value) => setFormData(prev => ({ ...prev, phone: formatPhoneNumber(value) }))}
      />
      ...
      <div className="border-t border-gray-200 pt-6">
        ...
        <div className="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus-within:border-orange-500 focus-within:ring-1 focus-within:ring-orange-500">
          <CardElement options={CARD_ELEMENT_OPTIONS} className="py-3 px-3" />
        </div>
      </div>
